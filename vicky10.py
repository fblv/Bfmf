...
km = LINETCR.LINE()
km.login(token="Eqsstw0tcrJE6U2SGy1f.w2y3st77l8p+Bfx9XcVvtW.9ZrsNJ6rZgkrBXKG10OPiJqgl0/jWmnfRMtz2hTpBN4=")

kt = LINETCR.LINE()
kt.login(token="EqLaWUVtvWNeTWBLh8cc.dmwV+8AvfCg9qISHOhivVa.kJdyL8NrEvoK5OtwFFpWrWIkcw+thTu6FyCI/OYgip0=")

kw = LINETCR.LINE()
kw.login(token="Eq7UxJfOSY0FHTMQrHzf.bMxsVktNlf4/LlMU1msQZW.AHpw7nYxOvSi+97irKfMkaCiv6t0o4DZaVX2GgIyOxE=")

kx = LINETCR.LINE()
kx.login(token="Eq972I44RgxpLL3chC63.o2XXqY54uxyx4WHdWleLeW.z1fkmH0vqyVE5aGpUkOagPjOnt6volg+hGVXsWvqYyk=")

ky = LINETCR.LINE()
ky.login(token="EqtZsBwCPHMefbdQxWH4.0aPixyJFVtPGnatN+0kpja.vZFWL+km5u5SSczAM7fseCfDZt1kbJVTsXRCE4/KFz4=")

kz = LINETCR.LINE()
kz.login(token="EqN9lwfrSpDXhzofVDP4.mltN4OiQZ5b5VUy8HPrkfa.Y5wtUIRrKF7VV8tYe7f107zMynH3RSeCVhnJ+7kZQl8=")
...
helpMessage =""" ☯️Pasukan ✍V҉i҉c҉k҉y ☯️
...
-- Namelock on/off
"""
KAC=[cl,ki,kk,kc,ks,km,kt,kw,kx,ky,kz]
DEF=[ki,kk,kc,ks,km,kt,kw,kx,ky,kz]
...
Gmid = kt.getProfile().mid
Hmid = kt.getProfile().mid
Imid = kt.getProfile().mid
...
wait = {
    ...
    "lang":"JP",
    "tag":True,
    "comment":"Auto Like By ☯️Pasukan ✍V҉i҉c҉k҉y ☯️",
    "commentOn":True,
    "commentBlack":{},
    "wblack":False,
    "dblack":False,
...
#==================================================
		...
		for ticket_id in n_links:
			if wait["atjointicket"] == True:
                ...
                kw.sendText(msg.to,Cmid)
                kx.sendText(msg.to,Dmid)
                ky.sendText(msg.to,Emid)
                kz.sendText(msg.to,Fmid)
            elif "Mid 1" == msg.text:
...
#----------------------------------------------------------------------------
    #-------------Fungsi Tag All Start---------------#
            elif msg.text in ["Next","Sahur","Kuchiyose no jutsu"]:
                  group = cl.getGroup(msg.to)
                  nama = [contact.mid for contact in group.members]
                    ...
                    if targets == []:
                        ...
                        for target in targets:
                          if target not in Bots:
                            try:
                                klist=[kz,ky,kx,kw,ks,kc,kk,ki,cl]
                                kicker=random.choice(klist)
        ...
        #----------------Fungsi Kick User Target Start----------------------#
                  ...
                  if msg.from_ in admin:
                       ...
                       if targets == []:
                           ...
                           for target in targets:
                                try:
                                    klist=[cl,ki,kk,kc,ks,km,kt,kw,kw,ky,kz]
                                    kicker=random.choice(klist)
                                    kicker.kickoutFromGroup(msg.to,[target])
                                    print (msg.to,[g.mid])
                                except:
...
#----------------------------------------------------------------------------
       #-------------Fungsi Respon Start---------------------#
                ...
                kw.sendText(msg.to,"7 Hadir boss..")
                kx.sendText(msg.to,"8 Hadir boss..")
                ky.sendText(msg.to,"9 Hadir boss..")
                kz.sendText(msg.to,"10 Hadir boss..")
